...
### 23.1 Phase 10E/10F immutable release cut

Phase 10E is source-only. It introduces `--local-release-descriptor <canonical-json>` as the sole managed-release selector. The `ploinky-release-v1` value couples exact raw Box and Node IDs, their exact digests, artifact and controller Ploinky commits, AgentLib `dd94929443033c0a43bf7569068ec1d2926dba35`, unique Router/media host ports, and a derived 64-hex release generation. Partial, extra, malformed, mutable, mixed, stale, or independently overridden inputs fail before mutation. Loose `--local-box-image-id`, `--local-node-image-id`, and `--local-media-port` inputs are retired.

The artifact/controller split is mandatory rather than a compatibility fallback. The artifact commit owns image source labels; the current controller commit owns admission and lifecycle. Both source trees must have the same AgentLib gitlink, global package pin, and Box dependency lock. A managed descriptor forbids independent `PLOINKY_AGENTLIB_REF` and `PLOINKY_RELEASE_GENERATION` values. False/missing coding selectors inspect and launch the descriptor's exact local Node ID without consulting the LLM-image override gate. True selectors remain bwrap/Seatbelt and perform zero Node-image activity.

Phase 10F must perform clean candidate builds from the accepted Phase 10E Ploinky head and the exact AgentLib pin; it must not retag, publish stable, or reuse Phase 10C as current. Both candidate images must carry the exact AgentLib OCI label, their native evidence must record it, and the candidate indexes must annotate it. The previous descriptor uses the Phase 10C Box/Node pair and artifact source `4e963bc7633dff333594d0d88b5ee5ed53dfa71e`, but uses the new controller source and unique rollback ports. Those fixed Phase 10C artifacts predate the AgentLib label and are admitted only by their exact IDs/digests plus the artifact source's aligned gitlink/package/lock proof. The current descriptor uses the rebuilt Box/Node IDs and digests, the new source as both artifact and controller, and a different Router/media port pair. Validate the pair with the descriptor distinctness contract before the managed Explorer drill. Phase 10A is not a managed rollback generation.
...
| Managed release selector | One canonical `ploinky-release-v1` descriptor couples immutable Box, Node, source, AgentLib, and port identities and derives the release generation. | No loose release environment or CLI input, retagging, image pull, old supervisor, or mixed-generation adoption can participate in cutover or rollback. |
...
| Immutable release | The public descriptor rejects partial/mutable/conflicting identities; false/missing OpenCode, Codex, and PI select its raw Node ID even without `llmRuntime.enabled`; true mode leaves that ID wholly dormant; status, lease, reconcile, route, provider, cancellation, and cleanup ownership all match its generation. Previous/current descriptors use distinct Router and media ports and share the exact attested AgentLib identity. |
